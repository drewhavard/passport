...
class AuthorizationControllerTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        DrewHavard\Passport\Passport::tokensCan([
            'scope-1' => 'description',
        ]);
        ...
        $controller = new DrewHavard\Passport\Http\Controllers\AuthorizationController($server, $response);
        ...
        $authRequest->shouldReceive('getScopes')->andReturn([new DrewHavard\Passport\Bridge\Scope('scope-1')]);
        ...
        $clients = Mockery::mock('DrewHavard\Passport\ClientRepository');
        $clients->shouldReceive('find')->with(1)->andReturn('client');

        $tokens = Mockery::mock('DrewHavard\Passport\TokenRepository');
    ...
    public function test_request_is_approved_if_valid_token_exists()
    {
        ...
        $authRequest->shouldReceive('getScopes')->once()->andReturn([new DrewHavard\Passport\Bridge\Scope('scope-1')]);
        ...
        $tokens = Mockery::mock('DrewHavard\Passport\TokenRepository');
        $tokens->shouldReceive('findValidToken')->with($user, 'client')->andReturn($token = Mockery::mock('DrewHavard\Passport\Token'));
